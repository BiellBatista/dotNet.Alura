...
namespace _05_XX_Arquitetura_Plugins.XYZ.Plugins
{
    public class RelatorioXML : IRelatorio<Boleto>
    {
        ...
        private void GravarArquivo(List<BoletosPorCedente> listaObjetos)
        {
            ...
            var caminhoArquivo = Path.Combine(PastaDestino, $"{typeof(BoletosPorCedente).Name}.xml");

            // Usar Reflection para obter propriedades do tipo genérico
            var objetoProperties = typeof(BoletosPorCedente).GetProperties();

            // Escrever os dados no arquivo XML
            var settings = new XmlWriterSettings { Indent = true };

            using (var xmlWriter = XmlWriter.Create(caminhoArquivo, settings))
            {
